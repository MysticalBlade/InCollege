...
FILE-CONTROL.
    SELECT USER-INPUT ASSIGN TO 'InCollege-test4.txt'
...
01  CONNECTION-TABLE.
    ...
    05  CONNECTION-ENTRY OCCURS 25 TIMES.
        ...
        10  CONN-STATUS        PIC X(01). *>For connection requests
            88 CONN-PENDING      VALUE 'P'.
            88 CONN-ACCEPTED     VALUE 'A'.
            88 CONN-REJECTED     VALUE 'R'.

01  WS-REQUEST-CHOICE          PIC X(01) VALUE SPACES.
01  WS-REQS-PROCESSED          PIC 99    VALUE 0.
01  WS-TEMP-CONN-INDEX         PIC 99    VALUE 0.
...
500-POST-LOGIN-OPERATIONS.
    PERFORM UNTIL NO-MORE-DATA
        ...
        PERFORM 700-DISPLAY-MESSAGE
        MOVE "Manage Connection Requests" TO MESSAGE-BUFFER
        PERFORM 700-DISPLAY-MESSAGE
...
910-SEND-CONNECTION-REQUESTS.
    ...
    *> If we get here, no request exists. Add a new one.
    IF CONNECTION-COUNT < MAX-CONNECTIONS
        ...
        MOVE 'P' TO CONN-STATUS(CONNECTION-COUNT)      *>Sets status to Pending
...
1000-MANAGE-REQUESTS.
    MOVE 0 TO WS-REQS-PROCESSED.

    MOVE "----------------------------------------" TO MESSAGE-BUFFER
    PERFORM 700-DISPLAY-MESSAGE.
    MOVE "--- Manage Pending Connection Requests ---" TO MESSAGE-BUFFER
    PERFORM 700-DISPLAY-MESSAGE

    PERFORM VARYING LOOP-INDEX FROM 1 BY 1
      UNTIL LOOP-INDEX > CONNECTION-COUNT

        IF FUNCTION TRIM(CONN-RECEIVER(LOOP-INDEX)) = FUNCTION TRIM(CURRENT-USER)
           AND CONN-PENDING(LOOP-INDEX)

           ADD 1 TO WS-REQS-PROCESSED

           MOVE SPACES TO MESSAGE-BUFFER
           STRING "Request # " FUNCTION NUMVAL-C(WS-REQS-PROCESSED)
                  ": From " FUNCTION TRIM(CONN-SENDER(LOOP-INDEX))
                  DELIMITED BY SIZE INTO MESSAGE-BUFFER
           PERFORM 700-DISPLAY-MESSAGE

           MOVE "  Accept (A) or Reject (R)?" TO MESSAGE-BUFFER
           PERFORM 700-DISPLAY-MESSAGE WITH NO ADVANCING.

           PERFORM 600-GET-USER-INPUT
           IF NO-MORE-DATA
               EXIT PARAGRAPH
           END-IF
           MOVE FUNCTION UPPER-CASE(FUNCTION TRIM(INPUT-BUFFER)) TO WS-REQUEST-CHOICE

           EVALUATE WS-REQUEST-CHOICE
               WHEN 'A'        *>connection is accepted
                   MOVE 'A' TO CONN-STATUS(LOOP-INDEX)
                   MOVE "  -> Connection accepted with " TO MESSAGE-BUFFER
                   STRING MESSAGE-BUFFER FUNCTION TRIM(CONN-SENDER(LOOP-INDEX))
                          DELIMITED BY SIZE INTO MESSAGE-BUFFER
                   PERFORM 700-DISPLAY-MESSAGE

               WHEN 'R'        *>connection is rejected
                   MOVE 'R' TO CONN-STATUS(LOOP-INDEX)
                   MOVE "  -> Connection rejected from " TO MESSAGE-BUFFER
                   STRING MESSAGE-BUFFER FUNCTION TRIM(CONN-SENDER(LOOP-INDEX))
                          DELIMITED BY SIZE INTO MESSAGE-BUFFER
                   PERFORM 700-DISPLAY-MESSAGE

               WHEN OTHER
                   MOVE "  -> Invalid choice. Connection still pending." TO MESSAGE-BUFFER
                   PERFORM 700-DISPLAY-MESSAGE
           END-EVALUATE
        END-IF
    END-PERFORM.

    IF WS-REQS-PROCESSED = 0
        MOVE "You have no pending connection requests." TO MESSAGE-BUFFER
        PERFORM 700-DISPLAY-MESSAGE
    END-IF.
    ...
    *> Refine the table to only include accepted requests
    PERFORM 1010-ACCEPTED-CONNECTIONS.
    PERFORM 960-SAVE-CONNECTIONS.

1010-ACCEPTED-CONNECTIONS.
    MOVE 0 TO WS-TEMP-CONN-INDEX.
    PERFORM VARYING LOOP-INDEX FROM 1 BY 1 UNTIL LOOP-INDEX > CONNECTION-COUNT
        IF CONN-STATUS(LOOP-INDEX) NOT = 'R'
           ADD 1 TO WS-TEMP-CONN-INDEX
           IF LOOP-INDEX NOT = WS-TEMP-CONN-INDEX
              MOVE CONNECTION-ENTRY(LOOP-INDEX) TO CONNECTION-ENTRY(WS-TEMP-CONN-INDEX)
           END-IF
        END-IF
    END-PERFORM.
    MOVE WS-TEMP-CONN-INDEX TO CONNECTION-COUNT.
    PERFORM VARYING LOOP-INDEX FROM CONNECTION-COUNT + 1 BY 1
      UNTIL LOOP-INDEX > MAX-CONNECTIONS
        MOVE SPACES TO CONNECTION-ENTRY(LOOP-INDEX)
    END-PERFORM.